...
Comprehensive examples demonstrating OfficeCLI capabilities for Word, Excel, PowerPoint, and PDF automation.
...
**PDF (.pdf):**
```bash
officecli view test.pdf --mode text          # Extract text
officecli view test.pdf --mode outline        # Page listing + metadata
officecli view test.pdf --mode stats          # Pages, chars, lines
officecli get test.pdf '/page[1]' --json      # Get page content
officecli extract-text test.pdf --with-offsets --json  # Text + offset mapping
```
...
- Page content access and navigation
- Offset→path mapping for AI agent positioning
- Text replacement and page deletion
...
| `extract-text` | Text + offset mapping | `officecli extract-text file.docx --with-offsets --json` |
