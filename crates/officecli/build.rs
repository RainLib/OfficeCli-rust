...
fn main() {
    ...
    generate_plugin_lint_schema_index(&workspace_dir);
...
/// Generate a lookup table for plugin linting. The JSON schemas are compiled
/// into the binary so installed releases do not depend on a checkout layout.
fn generate_plugin_lint_schema_index(workspace_dir: &Path) {
    let schema_root = workspace_dir.join("schemas/help");
    let mut files = Vec::new();
    collect_json_files(&schema_root, &mut files);
    files.sort();
    ...
    let mut generated = String::from(
        "// Generated by build.rs. Do not edit manually.\n\
         pub(super) fn schema_json(target: &str, element: &str) -> Option<&'static str> {\n\
         match (target, element) {\n",
    );
    for path in files {
        let relative = path
            .strip_prefix(&schema_root)
            .expect("schema file must be below the schema root");
        let mut components = relative.components();
        let Some(target) = components.next() else {
            continue;
        };
        ...
        writeln!(
            generated,
            "({:?}, {element:?}) => Some(include_str!({:?})),",
            target.as_os_str().to_string_lossy(),
            absolute.to_string_lossy()
        )
        .expect("writing generated schema index must succeed");
    }
    generated.push_str("_ => None,\n}\n}\n");

    let out_dir = PathBuf::from(env::var_os("OUT_DIR").unwrap());
    fs::write(out_dir.join("plugin_lint_schemas.rs"), generated).unwrap();
}
