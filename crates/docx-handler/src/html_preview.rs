        ...
        if let Ok(num_doc) = roxmltree::Document::parse(&num_xml) {
                ...
                .filter(|n| n.has_tag_name("abstractNum"))
            {
                if let Some(id) = abs
                    ...
                    .or_else(|| abs.attribute("w:abstractNumId"))
                {
                    let mut lvls = HashMap::new();
                    for lvl in abs.children().filter(|n| n.has_tag_name("lvl")) {
                        if let Some(ilvl) = lvl
                            .attribute((W_NS, "ilvl"))
                            .or_else(|| lvl.attribute("w:ilvl"))
                        {
                            let fmt = lvl
                                .children()
                            ...
                            let indent = lvl
                                ...
                                .and_then(|i| {
                                    i.attribute((W_NS, "left"))
                                        .or_else(|| i.attribute("w:left"))
                                })
            ...
            for num in num_doc.descendants().filter(|n| n.has_tag_name("num")) {
                if let Some(id) = num
                    ...
                    .or_else(|| num.attribute("w:numId"))
                {
                    if let Some(abs_ref) = num.children().find(|n| n.has_tag_name("abstractNumId"))
                    {
                        if let Some(val) = abs_ref
                            .attribute((W_NS, "val"))
                            .or_else(|| abs_ref.attribute("w:val"))
                        {
...
fn render_paragraph(
                ...
                if let Some((fmt, text, left_pt)) = levels.get(ilvl) {
                    ...
                    *count += 1;
                    ...
                    let glyph = match fmt.as_str() {
                        ...
                        "lowerLetter" => {
                            if *count >= 1 && *count <= 26 {
                                ((b'a' + (*count - 1) as u8) as char).to_string()
                            } else {
                                count.to_string()
                            }
                        }
                    ...
                    num_prefix = text.replace(
                        &format!("%{}", ilvl.parse::<usize>().unwrap_or(0) + 1),
                        &glyph,
                    );
                    num_indent = *left_pt;
                }
    ...
    if num_indent > 0.0 {
        styles.push(format!(
            "margin-left:{:.1}pt; text-indent:-15pt;",
            num_indent
        ));
    }
    ...
    let class_attr = if !num_prefix.is_empty() {
        " class=\"has-num\""
    } else {
        ""
    };
